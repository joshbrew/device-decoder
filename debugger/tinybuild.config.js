const config = {
    bundler: { //esbuild settings, set false to skip build step or add bundle:true to config object to only bundle (alt methods)
        ...
        outfile: 'dist/index',//"dist/index", //exit point file, will append .js as well as indicators like .esm.js, .node.js for other build flags
    ...
    server: {  //node server settings, set false to skip server step or add serve:true to config object to only serve (alt methods)
        ...
        watch: ['../GraphServiceRouter/','../BrainsAtPlay_Libraries/webgl-plot-utils'], //watch additional directories other than the current working directory
